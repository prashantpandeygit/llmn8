...
3. (the ui shows offline, and no buttons accessible).
4. go to the other folder exe.win-amd64-3.11 and launch the microchat.exe
5. the local api will launch within a time frame of 10-15 seconds.
...
8. after the model is downloaded, press the load model button.
...
3. npm install (then make changes if needed).
4. npm run build:win (to create the executable file for the ui, created in the dist folder).
5. get to the server directory.
6. create a virtual environment and install dependencies (uv package manager is used, then make changes if needed)
7. launch terminal for step 8.
